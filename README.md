...
│
├── config/                   # NEAT 配置文件目录
│   ├── neat_retail.cfg       # 散户 NEAT 配置（127输入，8输出）
│   ├── neat_retail_pro.cfg   # 高级散户 NEAT 配置（907输入，8输出）
│   ├── neat_whale.cfg        # 庄家 NEAT 配置（907输入，8输出）
...
|------|------|----------|------|----------|----------|------|
| **散户** | 10,000 | 2万 | 1.0x | 127 | 8 | 信息有限（买卖各10档+20笔成交），高数量 |
| **高级散户** | 100 | 2万 | 1.0x | 907 | 8 | 完整信息（100档+100笔成交） |
| **庄家** | 100 | 300万 | 1.0x | 907 | 8 | 大资金量，趋势跟随 |
| **做市商** | 400 | 1000万 | 1.0x | 964 | 41 | 提供流动性，双边挂单 |
...
**训练过程：**
1. 创建四个 NEAT 种群（散户、高级散户、庄家、做市商）
2. 每个 episode 运行指定数量的 tick
3. 每个 tick 执行：强平检查 → 鲶鱼行动（如启用）→ Agent 观察 → Agent 决策 → 订单撮合
...
| 角色 | Maker费 | Taker费 |
|------|---------|---------|
| 散户/高级散户 | 万2 (0.02%) | 万5 (0.05%) |
| 庄家/做市商 | -万1（返佣） | 万1 (0.01%) |
| 鲶鱼 | 0 | 0 |
...
```python
# Agent 配置
retail_count = 10_000             # 散户数量
retail_initial_balance = 20_000   # 散户初始资金
retail_leverage = 1.0             # 散户杠杆

whale_count = 100                 # 庄家数量
whale_initial_balance = 3_000_000 # 庄家初始资金

market_maker_count = 400          # 做市商数量
