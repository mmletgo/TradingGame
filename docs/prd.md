...
- **杠杆**: 10.0x
- **维持保证金率**: 0.05（5%）
- **信息获取**: 买卖各5档订单簿 + 最近100笔成交 + 最近100个tick历史
...
| 区间 | 数量 | 说明 |
|------|------|------|
| 0-9 | 10 | 买盘5档（价格归一化 + 数量归一化） |
| 10-19 | 10 | 卖盘5档（价格归一化 + 数量归一化） |
| 20-119 | 100 | 成交价格归一化 |
| 120-219 | 100 | 成交数量（带方向） |
| 220-223 | 4 | 持仓信息 |
| 224-226 | 3 | 挂单信息（价格归一化、数量、方向） |
| 227-326 | 100 | tick历史价格 |
| 327-426 | 100 | tick历史成交量 |
| 427-526 | 100 | tick历史成交额 |

**神经网络输出（3个）：**
- `[0]`: 动作选择（-1到1，等宽分6 bin：HOLD/PLACE_BID/PLACE_ASK/CANCEL/MARKET_BUY/MARKET_SELL）
- `[1]`: 价格偏移（-1到1，映射到 ±100 ticks）
- `[2]`: 数量比例（-1到1，映射到 [0, 1.0]）
...
| 120-219 | 100 | 成交数量（带方向） |
| 220-223 | 4 | 持仓信息 |
| 224-283 | 60 | 挂单信息（10买单×3 + 10卖单×3） |
| 284-383 | 100 | tick历史价格 |
| 384-483 | 100 | tick历史成交量 |
| 484-583 | 100 | tick历史成交额 |
| 584-591 | 8 | AS模型特征（见下表） |

**AS模型特征（索引584-591）：**

| 索引 | 名称 | 说明 |
|------|------|------|
| 584 | reservation_offset | 保留价格偏移 `(r - mid) / mid` |
| 585 | optimal_half_spread | 最优半点差比率 |
| 586 | sigma | 已实现波动率 |
| 587 | tau | 剩余时间比例 [0, 1] |
| 588 | kappa | 订单到达率（对数归一化） |
| 589 | inventory_risk | 库存风险 `gamma × sigma² × tau × inventory_ratio` |
| 590 | gamma_ratio | 风险厌恶系数比率 |
| 591 | spread_vol_ratio | 点差/波动率比率 |
...
├── config/                       # NEAT配置文件
│   ├── neat_retail_pro.cfg       # 高级散户（527输入，3输出，pop_size=240）
│   └── neat_market_maker.cfg     # 做市商（592输入，43输出，pop_size=100）
├── checkpoints/                  # 检查点
├── logs/                         # 日志
├── docs/                         # 文档
└── tests/                        # 测试
...
| 做市商 | 600 | 1,000万 | 10.0x | 0.05 | -万1 | 万1 |
...
|---------|----------|---------|---------|---------|---------|
| neat_retail_pro.cfg | 高级散户 | 527 | 3 | 10 | 240 |
