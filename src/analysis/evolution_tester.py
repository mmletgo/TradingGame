...
def _run_single_test_worker(params: dict[str, Any]) -> dict[str, Any]:
    ...
    for agent_type, population in trainer.populations.items():
        initial_balance = initial_balances.get(agent_type, 1.0)
        alive_count = 0
        total_equity = 0.0
        equities: list[float] = []
        ...
        if equities:
            ...
            avg_return_rate = (avg_equity - initial_balance) / initial_balance
        else:
            avg_return_rate = -1.0  # 全部爆仓
...
class EvolutionTester:
    ...
    def run_baseline_test(
        ...
        Args:
            generation: 代数
            num_runs: 运行次数
            episode_length: 每次运行的 episode 长度
            force: 是否强制重新运行（忽略缓存）
    ...
    def _summarize_results(
        ...
        for agent_type in AgentType:
            ...
            position_distributions: list[dict[str, int]] = []

            for run_result in results:
                species_result = run_result.get("species_results", {}).get(agent_type)
                if species_result:
                    return_rates.append(species_result["avg_return_rate"])
                    survival_rates.append(species_result["survival_rate"])
                    position_distributions.append(
                        species_result["position_distribution"]
                    )

            if return_rates:
                species_summary[agent_type] = {
                    "avg_return_rate": float(np.mean(return_rates)),
                    "std_return_rate": float(np.std(return_rates)),
                    "avg_survival_rate": float(np.mean(survival_rates)),
                    "std_survival_rate": float(np.std(survival_rates)),
                    "runs": len(return_rates),
                }
    ...
    def _compile_effectiveness_report(
        ...
        for agent_type in AgentType:
            ...
            effectiveness[agent_type] = {
                "baseline_return_rate": baseline_return,
                "comparison_return_rate": comparison_return,
