...
- **exploiter_training**：Exploiter vs 历史对手
...
class ArenaAllocation:
    ...
    exploiter_arena_ids: dict[AgentType, list[int]]
```
...
默认 27 个竞技场 × 2 个 episode = 54 个竞技场-episode 组合：

| 竞技场范围 | 数量 | 用途 |
|-----------|------|------|
| Arena 0-4 | 5 | 基准对战（全当前代） |
| Arena 5-8 | 4 | 散户泛化测试 |
| Arena 9-12 | 4 | 高级散户泛化测试 |
| Arena 13-16 | 4 | 庄家泛化测试 |
| Arena 17-20 | 4 | 做市商泛化测试 |
| Arena 21-24 | 4 | League Exploiter 训练 |
| Arena 25-26 | 2 | Main Exploiter 训练 |
...
```python
散户 Main 适应度 = 加权平均 {
    基准竞技场(Arena 0-4) × 权重 1.0,
    散户泛化测试竞技场(Arena 5-8) × 权重 0.8
}
```

总权重 = 5×2×1.0 + 4×2×0.8 = 16.4
