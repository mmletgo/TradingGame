...
- `start()` - 启动所有竞技场进程
- `monitor(progress_callback, check_interval)` - 监控所有竞技场的运行状态（非阻塞），只在所有竞技场最大 episode 增加时调用回调
...
- `elite_species_fitness: dict[AgentType, float]` - 各种群最精英 species 的平均适应度
...
- `get_global_summary()` - 获取全局汇总，包含各竞技场最新 episode 的 `elite_species_fitness` 平均值
