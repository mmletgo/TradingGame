...
checkpoints/multi_arena/
    config.pkl              # 全局配置
    arena_0/
        checkpoint.pkl      # 竞技场 0 的检查点
    arena_1/
        checkpoint.pkl      # 竞技场 1 的检查点
    ...
```
...
- `update_arena(arena_id, episode, populations, best_genomes)` - 更新单个竞技场的 checkpoint（原子写入，无锁）
- `get_migration_candidates(requesting_arena_id, count_per_arena)` - 遍历其他竞技场目录获取最佳个体
...
**6. 迁移后 GC (`_execute_migration_from_checkpoint()`)**

执行迁移后立即清理临时对象并强制垃圾回收。

**7. 检查点保存后 GC (`_save_arena_to_checkpoint()`)**

保存检查点后立即清理临时数据并强制垃圾回收。

**8. 每 episode 垃圾回收 (`arena_worker_autonomous()`)**
