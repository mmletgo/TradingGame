...
- `get_best_genomes(top_n)` - 获取各种群的最佳基因组（用于共享检查点）
- `inject_genomes(packets)` - 注入迁入的基因组（替换最差个体），注入后自动重建 trainer 的 agent_execution_order 和 agent_map
- `_inject_genome_to_population(population, genome)` - 将单个 genome 注入到种群中（替换最差个体），**注入后会重新划分物种**（调用 `speciate()`）以确保 NEAT 物种信息同步
- `get_checkpoint_data()` - 获取检查点数据
- `load_checkpoint_data(checkpoint)` - 加载检查点数据
- `stop()` - 停止竞技场（关闭线程池）

**物种同步机制（`_inject_genome_to_population`）：**
直接修改 `neat_pop.population` 后必须同步更新 `neat_pop.species`，否则会导致：
1. 物种的 `members` 仍然引用已删除的旧 genome
2. 新注入的 genome 没有被分配到任何物种
3. 当 `evolve()` → `neat_pop.run()` → `stagnation.update()` 调用 `s.get_fitnesses()` 时，返回的 fitness 列表中包含 None 值，导致 `'>' not supported between instances of 'NoneType' and 'float'` 错误

因此在替换 genome 后必须调用 `neat_pop.species.speciate()` 重新划分物种。
...
**6. 迁移注入后重建 trainer 状态 (`Arena.inject_genomes()`)**

注入迁移基因组后，由于 `_inject_genome_to_population()` 会清理旧 Agent 并创建新 Agent，必须重建 trainer 的内部状态：
- `trainer._register_all_agents()` - 重新注册所有 Agent 的费率
- `trainer._build_agent_map()` - 重建 agent_id -> agent 映射表
- `trainer._build_execution_order()` - 重建执行顺序列表
- `trainer._update_pop_total_counts()` - 更新种群计数

同时设置注入 genome 的 fitness（从 MigrationPacket 带来），避免进化时因 fitness 为 None 导致错误。
