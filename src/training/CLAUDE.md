...
**主要功能：**
- 创建和管理 NEAT 种群
- 从基因组创建对应类型的 Agent（散户/高级散户/多头庄家/空头庄家/做市商）
- 向量化评估适应度
- 执行 NEAT 进化算法
- 重置 Agent 账户状态
...
- `_build_execution_order()` - 构建 Agent 执行顺序列表（做市商->空头庄家->多头庄家->高级散户->散户）
...
- `config/neat_bull_whale.cfg` - 多头庄家（607 个输入节点，4 个输出节点）
- `config/neat_bear_whale.cfg` - 空头庄家（607 个输入节点，4 个输出节点）
...
- **Agent ID 唯一性**：每个种群类型使用不同的 ID 偏移量（散户=0，高级散户=1M，多头庄家=2M，空头庄家=3M，做市商=4M），确保不同种群的 agent_id 全局唯一，避免订单 ID 冲突。
