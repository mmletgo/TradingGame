...
adl/
├── __init__.py       # 模块导出
└── adl_manager.py    # ADL 管理器核心实现（ADLCandidate, ADLManager）
```
...
**返回值：** participant 强制转换为 Agent 类型

**异常：** 如果 participant 是 NoiseTrader，抛出 TypeError

#### `is_noise_trader -> bool`

检查候选者是否为噪声交易者。

**返回值：** True 表示是噪声交易者，False 表示是 Agent

### ADLManager (adl_manager.py)
...
#### `get_adl_price(current_price: float) -> float`
...
**返回值：** ADL 成交价格（即当前市场价格）

**设计理由：**
- 强平 不等于 破产：被强平时 Agent 可能还有正的净值（只是保证金率过低）
...
#### `calculate_adl_score(agent: Agent, current_price: float) -> ADLCandidate | None`
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `agent` | Agent | Agent 或 NoiseTrader 对象 |
| `current_price` | float | 当前市场价格 |
...
2. **计算开仓成本**：
   ```
   entry_cost = |quantity| x avg_price
   ```
...
6. **计算有效杠杆**：
   ...
   if equity <= 0:
       ...
       position_value = |quantity| x current_price
...
7. **计算 ADL 分数**：
   - 盈利者（pnl_percent > 0）：
     ```
     adl_score = pnl_percent x effective_leverage
     ```
   - 亏损者（pnl_percent <= 0）：
     ```
...
ADL 成交通过 `Account.on_adl_trade()` 或 `NoiseTraderAccount.on_adl_trade()` 方法更新账户：

**Account.on_adl_trade() 方法签名：**
```python
...
__all__ = ["ADLCandidate", "ADLManager"]
```
