...
3. **支持 Agent 和鲶鱼**：鲶鱼也可以作为 ADL 候选者参与减仓
...
在强平处理之前，预先计算所有 Agent 和鲶鱼的 ADL 分数：
...
for catfish in self.catfish_list:
    if not catfish.is_liquidated:
        # 鲶鱼使用庄家的杠杆率计算 ADL 分数
        candidate = self.adl_manager.calculate_adl_score(catfish, latest_price)
...
if candidate:
    ...
    print(f"是否为鲶鱼: {candidate.is_catfish}")
```
...
1. **预计算开销**
   - 每个 tick 计算一次所有 Agent 和鲶鱼的 ADL 分数
   - 时间复杂度：O(n)，n 为 Agent 和鲶鱼总数
...
- `CatfishConfig` - 鲶鱼配置，鲶鱼的初始资金和杠杆率

鲶鱼初始资金计算：
```
鲶鱼资金 = (做市商杠杆后资金 - 其他物种杠杆后资金) / 3
```
