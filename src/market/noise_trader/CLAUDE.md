...
噪声交易者是替代鲶鱼(catfish)的新机制，用于为市场提供随机性流动性和布朗运动价格特征。100个独立噪声交易者通过随机买卖行为，由中心极限定理保证净买卖量趋向正态分布，从而产生价格随机游走。

## 设计理念

1. **布朗运动模拟**：100个独立噪声交易者的净买卖量趋向正态分布，价格呈现随机游走特征
...
2. 行动时以 50% 概率决定买或卖
3. 下单量从对数正态分布采样：`max(1, int(lognormvariate(mu=3.0, sigma=1.0)))`
...
100个独立噪声交易者，各自以 50% 概率行动，行动时 50% 买/50% 卖：
