...
- `matching_engine.py` - 撮合引擎类（Python 实现，调用 Cython 撮合逻辑）
- `fast_matching.pyx` - Cython 加速的撮合核心逻辑及 FastMatchingEngine
...
#### `__init__(config: MarketConfig)`
初始化撮合引擎，创建 OrderBook 实例。
...
计算手续费，未注册 Agent 使用默认散户费率。

#### `match_limit_order(order) -> list`
限价单撮合，调用 `fast_match_orders`，剩余数量挂入订单簿。

#### `match_market_order(order) -> list`
市价单撮合，调用 `fast_match_orders`，未成交部分丢弃。

#### `process_order(order) -> list`
处理订单入口，根据 `order.order_type` 分派：
- `OrderType.LIMIT (1)` -> `match_limit_order`
- `OrderType.MARKET (2)` -> `match_market_order`

#### `cancel_order(order_id: int) -> bint`
撤单，返回是否成功。

**Properties：**
- `tick_size: float` - 获取最小变动单位
- `next_trade_id: int` - 获取下一个成交ID
- `fee_rates: dict` - 获取费率配置
- `orderbook` - 获取订单簿实例（通过 public 属性直接访问）
