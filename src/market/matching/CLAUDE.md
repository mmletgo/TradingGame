...
└── fast_matching.pyx        # Cython 加速实现（FastTrade + FastMatchingEngine + fast_match_orders）
```
...
4. 返回 `amount x rate`（可能为负数，表示 maker rebate）
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `orderbook` | OrderBook | OrderBook 实例 |
| `order` | Order | 待撮合订单 |
| `fee_rates` | dict[int, tuple[float, float]] | agent_id -> (maker_rate, taker_rate) |
| `next_trade_id` | int | 下一个成交 ID |
| `is_limit_order` | bint | 是否为限价单（True=需要价格检查，False=市价单） |
...
```
成交金额 = 成交价格 x 成交数量
taker 手续费 = 成交金额 x taker 费率
maker 手续费 = 成交金额 x maker 费率
```
...
- 成交金额 = 100 x 10000 = 1,000,000
- 买方（散户 A）手续费 = 1,000,000 x 0.0005 = 500（taker）
- 卖方（做市商 B）手续费 = 1,000,000 x (-0.0001) = -100（maker，获得奖励）
