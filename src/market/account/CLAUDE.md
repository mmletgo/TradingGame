...
**C 级别属性（cdef public，声明于 position.pxd）：**
| 属性 | 类型 | 说明 |
|------|------|------|
...
#### `update(side: int, quantity: int, price: double) -> double`
...
2. **持有多头（quantity > 0）**
   - **买入（加多仓）**：
     - 新均价 = `(旧数量 x 旧均价 + 新数量 x 新价格) / (旧数量+新数量)`
     - `quantity += quantity`
   - **卖出且数量 < 持仓（减多仓）**：
     - 已实现盈亏 = `(价格 - 均价) x 成交数量`
     - `quantity -= quantity`
...
#### `get_unrealized_pnl(current_price: double) -> double`

计算浮动盈亏。

**公式：** `(current_price - avg_price) x quantity`

**说明：**
- 多头：价格上涨时浮动盈亏为正
- 空头：价格下跌时浮动盈亏为正

#### `get_margin_used(current_price: double, leverage: double) -> double`

计算占用的保证金。

**公式：** `|quantity| x current_price / leverage`
...
**公式：** `equity / (|position.quantity| x current_price)`
...
**返回值：** 无持仓时返回 INFINITY（C 语言的无穷大）

#### `check_liquidation(current_price: double) -> bint`

检查是否需要强制平仓。

**返回值：** Cython 的 bint 类型（C 语言的布尔值）
...
equity = balance + unrealized_pnl
  |
position_value = |position.quantity| x current_price
  |
...
**占用保证金**：
```
margin_used = |position.quantity| x current_price / leverage
```

**净值**：
```
equity = balance + position.get_unrealized_pnl(current_price)
       = balance + (current_price - position.avg_price) x position.quantity
```

**保证金率**：
```
margin_ratio = equity / (|position.quantity| x current_price)
```
...
**成交价格**：
- 直接使用当前市场价格
- 设计原因：强平 不等于 破产，被强平时 Agent 可能还有正净值（仅保证金率过低）
...
if position.quantity > 0:
    realized_pnl = (price - position.avg_price) x actual_quantity
    position.quantity -= actual_quantity

# 空头被减仓
else:
    realized_pnl = (position.avg_price - price) x actual_quantity
    position.quantity += actual_quantity
...
| 配置项 | 类型 | 高级散户 | 做市商 |
|--------|------|----------|--------|
| `initial_balance` | float | 20,000 | 10,000,000 |
| `leverage` | float | 1.0 | 1.0 |
| `maintenance_margin_rate` | float | 0.5 (50%) | 0.5 (50%) |
| `maker_fee_rate` | float | 0.0002 (万2) | -0.0001 (负万1，返佣) |
| `taker_fee_rate` | float | 0.0005 (万5) | 0.0001 (万1) |
...
**降级策略**：如果 Cython 模块未编译，`FastAccount` 为 `None`，允许代码继续运行（使用 Python 版本的 Account）。
...
trade = Trade(
    ...
    buyer_id=1,
    seller_id=2,
    buyer_fee=0.5,  # 10 x 100 x 0.0005
    seller_fee=0.2,  # 10 x 100 x 0.0002
    is_buyer_taker=True,
    timestamp=0.0
)
...
# 计算净值
equity = account.get_equity(current_price)
# equity = 5000.0 + (100.0 - 95.0) x 1000 = 10000.0

# 计算保证金率
margin_ratio = account.get_margin_ratio(current_price)
# margin_ratio = 10000.0 / (1000 x 100.0) = 0.1
