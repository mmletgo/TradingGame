...
- `_input_buffer: np.ndarray` - 预分配的神经网络输入缓冲区（散户 67，高级散户/庄家 607，做市商 634）
...
重写基类方法，使用更大的输入缓冲区（634 = 604 + 30 挂单信息）。
...
### 庄家神经网络输入（607 个值）
庄家可以看到完整的100档订单簿和最近100笔成交，与高级散户相同的输入维度。
...
- `_input_buffer`: 神经网络输入（607维，做市商634维，散户67维）
