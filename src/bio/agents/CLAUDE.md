...
- 代码复用：散户继承高级散户的 decide 方法，仅限制可见市场数据
...
| `retail_pro.py` | 221 | 高级散户 Agent（实现散户/高级散户通用的 decide 方法）|
| `retail.py` | 176 | 散户 Agent（仅重写 observe 方法限制可见市场数据）|
| `whale.py` | 233 | 庄家 Agent（实现与散户相同的 decide 方法）|
...
定义散户/高级散户/庄家可执行的所有交易动作。做市商不使用 ActionType，直接执行双边挂单操作。
...
Agent 基类是所有四种 Agent 类型的父类，提供通用的属性和方法。
...
| `_input_buffer` | np.ndarray | 预分配的神经网络输入缓冲区（散户 127，高级散户/庄家 907，做市商 964）|
...
- `_input_buffer`: 907 维（散户 127，做市商 964）
...
- **WhaleAgent**: 8 个输出节点，6 种动作（与散户相同）
- **MarketMakerAgent**: 41 个输出节点，直接输出买卖双边订单参数

各子类重写 `execute_action` 实现特定行为：
- **RetailAgent/RetailProAgent/WhaleAgent**: PLACE_BID/PLACE_ASK 先撤旧单再挂新单
...
散户代表市场中数量最多的交易参与者，初始资金较少（2 万），只能看到有限的市场数据（买卖各 10 档订单簿、最近 10 笔成交、最近 20 个 tick 历史）。

**关键属性：**
- `ORDERBOOK_DEPTH = 10` - 可见订单簿档位数
- `TRADE_HISTORY_SIZE = 10` - 可见成交笔数
- `TICK_HISTORY_SIZE = 20` - 可见 tick 历史数量
- `INPUT_SIZE = 127` - 神经网络输入维度

**继承关系：** 继承自 RetailProAgent，仅重写 `observe` 方法限制可见市场数据。

**decide 方法：** 继承 RetailProAgent 的 decide 方法（8 个输出节点，6 种动作）。

**execute_action 方法：** 重写父类方法，PLACE_BID/PLACE_ASK 先撤旧单再挂新单。

**输入向量结构（127 维）：**
- 0-19: 买盘 10 档（价格归一化 + 数量）
- 20-39: 卖盘 10 档（价格归一化 + 数量）
- 40-49: 成交价格 10 笔
- 50-59: 成交数量 10 笔
- 60-63: 持仓信息（4 个值）
- 64-66: 挂单信息（3 个值）
- 67-86: tick 历史价格（20 个）
- 87-106: tick 历史成交量（20 个）
- 107-126: tick 历史成交额（20 个）

### 2. RetailProAgent - 高级散户

高级散户代表市场中具有更多信息优势的散户交易者。与普通散户相比，可以看到完整的 100 档订单簿和 100 笔成交。

**decide 方法：** 实现散户/高级散户通用的决策逻辑。
...
庄家代表市场中拥有大量资金的参与者，初始资金 3M，可以看到完整的 100 档订单簿和 100 笔成交。

**decide 方法：** 与 RetailProAgent 相同（8 个输出节点，6 种动作）。

**execute_action 方法：** PLACE_BID/PLACE_ASK 先撤旧单再挂新单，其他动作使用父类实现。

**输入向量结构：** 与高级散户相同（907 维）。

### 4. MarketMakerAgent - 做市商
...
| 区间 | 散户（127）| 高级散户/庄家（907）| 做市商（964）|
|------|-----------|------------------|-------------|
| 买盘 | 0-19（10档）| 0-199（100档）| 0-199（100档）|
| 卖盘 | 20-39（10档）| 200-399（100档）| 200-399（100档）|
| 成交价格 | 40-49（10笔）| 400-499（100笔）| 400-499（100笔）|
| 成交数量 | 50-59（10笔）| 500-599（100笔）| 500-599（100笔）|
| 持仓信息 | 60-63（4）| 600-603（4）| 600-603（4）|
| 挂单信息 | 64-66（3）| 604-606（3）| 604-663（60）|
| tick 历史价格 | 67-86（20）| 607-706（100）| 664-763（100）|
| tick 历史成交量 | 87-106（20）| 707-806（100）| 764-863（100）|
| tick 历史成交额 | 107-126（20）| 807-906（100）| 864-963（100）|

### 神经网络输出向量结构对比

| 索引 | 散户/高级散户/庄家（8）| 做市商（41）|
|------|---------------------|-------------|
...
| 函数名 | 功能 | 输入维度 |
|--------|------|---------|
| `fast_observe_retail()` | 构建散户的神经网络输入向量 | 127 |
| `fast_observe_full()` | 构建高级散户/庄家的神经网络输入向量 | 907 |
...
- **基类（散户/高级散户/庄家）**: 3 个值（价格归一化、数量、方向）
