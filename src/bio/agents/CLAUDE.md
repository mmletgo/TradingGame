...
| `_input_buffer` | np.ndarray | 预分配的神经网络输入缓冲区（高级散户 67，做市商 132）|
...
- 20-23: 持仓信息（4 个值）
- 24-26: 挂单信息（3 个值）
- 27-46: tick 历史价格（20 个）
- 47-66: tick 历史成交量（20 个）

做市商重写此方法使用 132 维缓冲区（包含 60 个挂单信息和 8 个 AS 特征）。
...
**observe 方法：** 重写基类方法，使用更大的输入缓冲区（132 维）。

**输入向量结构（132 维）：**
- 0-9: 买盘 5 档（价格归一化 + 数量）
- 10-19: 卖盘 5 档（价格归一化 + 数量）
- 20-23: 持仓信息（4 个值）
- 24-83: 挂单信息（60 个值：10 买单 x 3 + 10 卖单 x 3）
- 84-103: tick 历史价格（20 个）
- 104-123: tick 历史成交量（20 个）
- 124-131: AS 模型特征（8 个值，见下表）

**AS 模型特征（索引 124-131）：**

| 索引 | 名称 | 说明 |
|------|------|------|
| 124 | reservation_offset | 保留价格偏移（`(r - mid) / mid`，Avellaneda-Stoikov 公式计算的保留价格与中间价之差的相对值）|
| 125 | optimal_half_spread / mid_price | 最优半点差比率（AS 公式给出的最优买卖半点差除以中间价）|
| 126 | sigma | 已实现波动率（近期成交价格的标准差，用于 AS 公式输入）|
| 127 | tau | 剩余时间比例（当前 tick 距 episode 结束的时间比例，`[0, 1]`）|
| 128 | kappa | 订单到达率（对数归一化：`log(kappa + 1) / log(kappa_max + 1)`）|
| 129 | inventory_risk | 库存风险（`gamma * sigma^2 * tau * inventory_ratio`，AS 模型的库存惩罚项）|
| 130 | gamma / max_gamma | 风险厌恶系数比率（当前 gamma 除以种群最大 gamma）|
| 131 | spread / (sigma + 1e-8) | 点差/波动率比率（当前报价点差除以已实现波动率，衡量点差相对波动率的水平）|
...
| 持仓信息 | 20-23（4）| 20-23（4）|
| 挂单信息 | 24-26（3）| 24-83（60）|
| tick 历史价格 | 27-46（20）| 84-103（20）|
| tick 历史成交量 | 47-66（20）| 104-123（20）|
| AS 特征 | -（无）| 124-131（8）|
...
| 函数名 | 功能 | 输入维度 |
|--------|------|---------|
| `fast_observe_full()` | 构建高级散户的神经网络输入向量 | 67 |
| `fast_observe_market_maker()` | 构建做市商的神经网络输入向量 | 132 |
