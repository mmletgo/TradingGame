...
class RetailProAgent(Agent):
    ...
    def __init__(self, agent_id: int, brain: Brain, config: AgentConfig, event_bus: EventBus) -> None:
        ...
        调用父类构造函数，设置类型为 RETAIL_PRO，并初始化动作处理器。
        ...
        Args:
            ...
            event_bus: 事件总线
        """
        super().__init__(agent_id, AgentType.RETAIL_PRO, brain, config, event_bus)
        self._init_action_handlers()

    def _init_action_handlers(self) -> None:
        """初始化动作处理器字典

        先调用父类初始化基础动作处理器，再覆盖 PLACE_BID 和 PLACE_ASK 为高级散户特定实现。
        """
        super()._init_action_handlers()
        # 覆盖 PLACE_BID 和 PLACE_ASK 为高级散户特定实现（先撤旧单再挂新单）
    ...
    def execute_action(self, action: ActionType, params: dict[str, Any], event_bus: EventBus) -> None:
        ...
        高级散户的 PLACE_BID 和 PLACE_ASK 使用特定实现（先撤旧单再挂新单），
        其他动作继承自父类的分发表。
