...
**枚举值：**
- `RETAIL_PRO` - 高级散户：数量 4,800（10子种群×480），资金 2万，可看到深层订单簿（5档）
- `MARKET_MAKER` - 做市商：数量 1,200（6子种群×200），资金 1,000万，必须双边挂单提供流动性
...
|-----------|------|---------|------|------------|---------|---------|
| 高级散户 (RETAIL_PRO) | 4,800 | 2万 | 10.0 | 0.05 | 0.0002 (万2) | 0.0005 (万5) |
| 做市商 (MARKET_MAKER) | 1,200 | 1,000万 (10M) | 10.0 | 0.05 | -0.0001 (负万1) | 0.0001 (万1) |

**详细说明：**

- **count**：该类型 Agent 的种群数量
  - 高级散户 4,800（10子种群×480），代表交易者群体
  - 做市商 1,200（6子种群×200），提供足够的流动性
...
- **retail_pro_sub_population_count**：高级散户子种群数量
  - 将 4,800 个高级散户拆分为 12 个子种群，每个 400 个
...
|--------|----------|---------|---------|---------|---------|
| neat_retail_pro.cfg | 高级散户 | 67 | 8 | 10 | 480 |
| neat_market_maker.cfg | 做市商 | 132 | 43 | 10 | 200 |

### 输入输出节点说明

**高级散户 (RETAIL_PRO) - 67 输入，3 输出：**
- 输入：
  ...
  - tick历史价格归一化(20)
  - tick历史成交量归一化(20)
  - 总计：10 + 10 + 4 + 3 + 20 + 20 = 67
- 输出：动作选择(1) + 价格偏移(1) + 数量比例(1) = 3个

**做市商 (MARKET_MAKER) - 132 输入，43 输出：**
- 输入：
  - 5档买盘价格归一化(5) + 5档买盘数量归一化(5) = 10
  - 5档卖盘价格归一化(5) + 5档卖盘数量归一化(5) = 10
  - 持仓信息(4)
...
- **quantity_mu / quantity_sigma**：
  - mu=12.0, sigma=1.0：默认参数，单笔均值约 268,337 单位，400个噪声交易者每 tick 总成交量约 5,360 万单位，噪声力量约为散户现实定向力量的 3.4 倍，既主导价格走势又不会击穿做市商双边挂单
