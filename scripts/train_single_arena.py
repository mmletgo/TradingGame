...
与 MultiArenaTrainer 的区别：单进程串行执行竞技场，充分利用 OpenMP 多核并行推理。
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="NEAT AI 交易模拟竞技场 - 单进程多竞技场训练模式",
    ...
    print("=" * 70)
    print("NEAT AI 交易模拟竞技场 - 单进程多竞技场训练模式")
    print("=" * 70)
    ...
    def checkpoint_callback(generation: int) -> None:
        if args.checkpoint_interval > 0 and generation % args.checkpoint_interval == 0:
            checkpoint_path = f"checkpoints/single_arena_gen_{generation}.pkl"
    ...
    try:
        ...
        # 保存紧急检查点
        emergency_path = (
            f"checkpoints/emergency_single_arena_gen_{trainer.generation}.pkl"
        )
